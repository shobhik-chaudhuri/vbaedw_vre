...
-- Creates the materialized view for the total number of participants for the USB Dashboard
CREATE MATERIALIZED VIEW MV_PARTICIPANTS_SUMMARY
REFRESH COMPLETE
ON DEMAND
START WITH sysdate
...
SELECT G.ACTUAL_DATE,
       CASE 
         WHEN o.outbase_site_name IN ('Australia', 'China', 'Foreign Workload', 'Guam', 'Indonesia', 'Japan', 'Korea',
                                      'Kuala Lumpur, Malaysia', 'Kure Island', 'Laos', 'Manila', 'Manila RO', 'Mayaguez',
                                      'Micronesia', 'New Zealand', 'Okinawa', 'Palau', 'Saipan', 'Samoa', 'Sepulveda', 
                                      'Sepulveda VA Clinic', 'Singapore', 'Taiwan', 'Thailand', 'Vancouver', 'Vietnam')
         THEN 'Y'
         ELSE 'N'
       END AS ForeignCaseFlag,
       CASE
         WHEN TRUNC(g.actual_date) = TRUNC(SYSDATE) THEN 'Number of Participants - today'
         ...
         ELSE NULL
       END AS Day_Label,
       COUNT(DISTINCT C.case_id) AS num_participants
...
INNER JOIN DW_DIM.EFFECTIVE_STATUS_INDICATOR_DIM E 
    ...
    AND E.EFFECTIVE_STATUS_IND_CODE <> 'D'
INNER JOIN DW_DIM.DATE_DIM D3 
    ON A.CASE_STATUS_ENTRY_DATE_SID = D3.DATE_DIM_SID
INNER JOIN DW_DIM.DATE_DIM D4
    ON A.CASE_STATUS_CLOSE_DATE_SID = D4.DATE_DIM_SID
    AND COALESCE(TRUNC(D4.ACTUAL_DATE), SYSDATE) >= '01-OCT-2013'
...
LEFT OUTER JOIN (
    SELECT a.*, RANK() OVER (PARTITION BY outbase_site_lctn_id ORDER BY begin_effective_date DESC) AS rn
